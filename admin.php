...
<br>       
  <table class="table table-striped">
    ...
    $vid_url = "videos/";
    $result = mysqli_query($con,"SELECT * FROM temp order by id desc");
    ...
    while($row=mysqli_fetch_array($result))
    {
        ...
        ?>
      <tr>
        <td> <video width="400" height="225" controls>
            <source src="<?php echo $video; ?>" type="video/<?php echo $type; ?>">
            Your browser does not support the video tag.
        </video></td>
        <td><?php echo $row["title"]; ?></td>
        <td><?php $id= $row["userid"]; 
    $results = mysqli_query($con,"SELECT * FROM user where id= '$id'");
    $rows=mysqli_fetch_row($results); 
        ?>  
        <form action="#" method="post">
     <input type="text" value="<?php echo $rows[0]; ?>" style="display: none;" name="userid">
     <button type="submit" class="btn btn-default btn-sm" style="border: none; color: blue;" name="user"><?php echo $rows[1]; ?></button></td>
            <input type="text" value="<?php echo $_SESSION['videoid']; ?>" style="display: none;" name="id">
        <td><button type="submit" class="btn btn-success" name="upload">Approve</button>
            <button type="submit" class="btn btn-danger" name="delete">Delete</button><br><br>
            <button type="submit" class="btn btn-warning" name="block">Block User</button></td>
      </form>
      </tr>
  <?php } ?>
...
if(isset($_POST['upload'])){
    ...
    echo "<script>alert('This Video has been uploaded');window.location.href='admin.php';</script>";
}
if(isset($_POST['delete'])){
    $id=$_POST['id'];
        $con= mysqli_connect("localhost","root","","foodtube");
    $query= mysqli_query($con,"delete from temp where id='$id'");
    echo "<script>alert('This Video has been deleted');window.location.href='admin.php';</script>";
}
if(isset($_POST['block'])){
    $id=$_POST['id'];
        $con= mysqli_connect("localhost","root","","foodtube");
    $result = mysqli_query($con,"SELECT * FROM temp where id='$id'");  
    $row=mysqli_fetch_row($result);
    $userid=$row[5];

    $results = mysqli_query($con, "UPDATE user SET status = 'blocked' WHERE id = '$userid'");
        $queries= mysqli_query($con,"delete from temp where id='$id'");
    echo "<script>alert('This User has been blocked');window.location.href='admin.php';</script>";
}
if(isset($_POST['user'])){
