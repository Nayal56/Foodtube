...
$expertid= $_SESSION["id"];
    $result = mysqli_query($con,"SELECT * FROM message where expertid='$expertid' order by id desc");
...
if($ures==$row["userid"]){
    ...
    $r=$row["userid"];
    $query=mysqli_query($con,"Select * from user where id=$r");
    $rw=mysqli_fetch_row($query);
  echo '<tr><td>'.$rw[1].'</td><td>'.$row["message"].'</td><form action="#" method="post">
    <input type="text" name="id" value="'.$row["userid"].'" style="display: none;">
        <td><button type="submit" class="btn btn-warning" name="message">Message</button></td>
      </tr>
  </form>';
$ures=$row["userid"];
